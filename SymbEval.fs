...
let symbolic_execution (R_in : RULE) (steps : int) : int * int option * RULE =
    ...
    if (steps < 0) then failwith "SymbEval.symbolic_execution: number of steps must be >= 0"
    ...
    let rec traverse i conditions R (S, env, C) =
        ...
        let check_invariants invs S0 conditions updates =
            let check_one (inv_id, t) =
                ...
                then met inv_id
                else if smt_formula_is_false sign TopLevel.smt_ctx t' then definitely_violated inv_id conditions updates t t'
