...
let location_to_string : LOCATION -> string = function
    (f, xs) -> sprintf "%s (%s)" f (String.concat ", " (List.map value_to_string xs))
...
let apply_update_map (S : STATE) (U : UPDATE_MAP) =
    ...
    let apply_to_dynamic_state (dS : DYNAMIC_STATE) (U : UPDATE_MAP) =
            Map.fold
                ...
                dS U
    in  { _signature = S._signature; _carrier_sets = S._carrier_sets; _static = S._static; _dynamic = apply_to_dynamic_state S._dynamic U; _dynamic_initial = S._dynamic_initial }
